...
- **Đấu (arena)**: "Rủ đấu" — chọn **Kiểu chơi** (Trắc nghiệm / Hangman 🎪) + bộ + số câu + cược → người kia NHẬN LỜI (cược lại được, cột `stake_acceptor`) mới bắt đầu → cả hai làm **cùng 1 bộ câu hỏi** (sinh 1 lần lưu chung, mảng `questions` jsonb — KHÔNG có cột `game` riêng, kiểu chơi tự suy ra từ `questions[0].kind`) → thấy tiến độ đối thủ ~trực tiếp → xong hiện thắng(👑)/thua(🥈)/hòa(🤝) + cược + lịch sử. Chữ dùng "Rủ đấu" (KHÔNG dùng "thách đấu" — user thấy gắt). **Nhấn giữ 1 trận trong lịch sử** (`data-longpress="duel-stake"`) → popup `openDuelStakeModal` chi tiết ai cược gì + ai phải làm gì (winStake = `win===challenger?stake:stake_acceptor`, khớp `duelResultView`).
  ...
  - **Giải ô chữ 🧩** (`buildCrosswordQuestions`/`crosswordGridHTML`/`duelCrosswordBody`/`dCheckCrossword`): mỗi câu là `{kind:'crossword',en,vi,row,col,dir,num}` — `buildCrosswordQuestions` tự XẾP Ô CHỮ (đặt từ dài trước, mỗi từ sau tìm vị trí GIAO chữ cái với từ đã đặt, ưu tiên giao nhiều nhất; từ không giao được ai thì đặt riêng 1 hàng trống `freeRow` để KHÔNG rớt từ nào đã chọn — chấp nhận vài "đảo" tách rời). Đã tự kiểm chứng bằng `preview_eval` (không đè sai chữ cái, không toạ độ âm, giữ đủ số từ) trước khi tin dùng — xem lại thuật toán này nếu cần sửa, đừng đoán mò. Trình bày các câu theo **thứ tự số thứ tự** (1 Ngang/Dọc, 2, 3…), KHÔNG random như quiz. Cả 2 người **giải ĐỘC LẬP cùng 1 bảng** (như Hangman) — mỗi lần trả lời 1 từ (ô nhập text, không gõ từng chữ vào ô lưới — bàn phím ảo per-cell quá phức tạp/dễ vỡ trên mobile nên cố tình đơn giản hoá). `crosswordGridHTML(du, p.idx)` vẽ LẠI TOÀN BỘ bảng mỗi lần render, hiện chữ cho các câu `index < p.idx` (đã qua — kể cả trả lời SAI vẫn hiện đáp án đúng lên bảng để giữ ô chữ nhất quán làm ngữ cảnh cho từ giao nhau), câu hiện tại/chưa tới để trống. Không cần cột DB mới (dùng lại `questions` jsonb).
