...
- **`grammar.js`**: `window.GRAMMAR` = dữ liệu NGỮ PHÁP tĩnh, **viết tay & kiểm chuẩn** (KHÔNG sinh bằng AI — kiến thức phải đúng). Đợt 1 = 12 thì (`group:present/past/future`). Mỗi chủ điểm: `formula{aff,neg,ques}`, `usage[]`, `signals[]`, `examples[{en,vi}]`, `practice[]` (type `conj` = chia động từ điền chỗ trống có `answer`+`alts`; type `mc` = trắc nghiệm `options`+`answer` index). Đợt sau chỉ THÊM group `conditional/passive/reported/comparison` vào file — không đổi engine (`GRAMMAR_GROUPS` tự hiện nhóm mới). File tĩnh, UTF-8, không nằm trong Supabase.
